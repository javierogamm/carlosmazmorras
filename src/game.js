...
function renderConfigIconPreview(hex){const preview=document.getElementById('configIconPreview');if(!preview)return;const c=preview.getContext('2d');c.clearRect(0,0,50,50);if(!hex)return;try{const data='data:image/png;base64,'+hexToBase64(hex.startsWith('#')?hex.slice(1):hex),img=configIconImage(data);const draw=()=>{c.clearRect(0,0,50,50);c.drawImage(img,0,0,50,50)};if(img.complete&&img.naturalWidth)draw();else img.onload=draw}catch(e){configIconStatus.textContent='No se pudo previsualizar el icono guardado.'}}
