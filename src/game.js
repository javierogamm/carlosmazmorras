...
function describeItem(item){item.defenseStat=item.defenseStat||inferWeaponDefenseStat(item);
 ...
 // item.effects is the legendary-equipment flavor-text list ({name,desc})
 // for regular gear - potions repurpose the same field name for their own
 // composable effects list (already rendered above via describePotionEffects),
 // so it's skipped here to avoid rendering it as bogus "undefined: undefined" lines.
 if(item.type!=='potion')for(const e of item.effects||[])lines.push(`<span class="effectLine">✦ ${e.name}: ${e.desc}</span>`);
 return lines.join('');
}
...
// ---- Potion casting: reuses the exact same composable-effects engine as
// skills (effectSourceDef/applyEffectComponent/applySkillEffectsList) -----
// A potion "cast" just registers itself in activePotionCast for the
// duration of the call, so the identical enemy/area/self dispatch a skill
// goes through also decides whether THIS potion needs a target (thrown) or
// applies instantly (drunk) - see effectsListTargetModeFor.
function potionCastId(item){return `potion:${item.id}`}
function potionLevel(item){return Math.max(1,Number(item.itemLevel)||potionTierLevel(item.rarity)||1)}
function beginPotionCast(item){activePotionCast={id:potionCastId(item),name:item.name,effects:item.effects||[],level:potionLevel(item)};return activePotionCast.id}
function endPotionCast(){activePotionCast=null}
...
let activePotionCast=null; // {id,name,effects,level}
function effectSourceDef(id){return skillDefs[id]||(activePotionCast&&activePotionCast.id===id?activePotionCast:null)}
function effectSourceLevel(id){return skillDefs[id]?skillLevel(id):(activePotionCast&&activePotionCast.id===id?activePotionCast.level:1)}
...
function confirmAreaTarget(){
 ...
 const ok=pendingTargetAction.kind==='potion'?resolveTargetedPotion(pendingTargetAction.potionId,x,y):resolveTargetedSkill(pendingTargetAction.slot,x,y);
...
function currentConfigItemJson(){
 ...
 if(itemType==='potion'){
  const name=configNombre.value.trim()||'Poción sin nombre',effects=JSON.parse(JSON.stringify(window.currentPotionEffectsDraft||[])),range=Number(document.getElementById('configPotionRange')?.value)||0;
  const desc=describePotionEffects({type:'potion',effects})||'Poción configurada.';
  return {type:'potion',name,slot:'consumable',rarity,label:tierDefs[rarity]?.label||rarity,itemLevel:ilvl,score:ilvl*8,icon:window.currentConfigIconHex||'',iconShape:'vial',effects,range,skillIds:[],stats:'',affixes:[],passives:[],hidden,desc}
 }
...
document.getElementById('game').addEventListener('click',ev=>{
 ...
 if(pendingTargetAction){
  if((pendingTargetAction.kind==='skill'||pendingTargetAction.kind==='potion')&&pendingTargetAction.mode==='area'){
