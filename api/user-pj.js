...
module.exports=async(req,res)=>{
 try{
  ...
  if(req.method==='GET'){
   ...
   const scoreSelect='id,created_at,nombre,pj_name,pj_status,pj_score,last_use,feats,soulseeker,max_floor_reached:pj_json->>maxFloorReached,level:pj_json->player->>level,class_name:pj_json->player->>className,race_name:pj_json->player->>raceName,difficulty:pj_json->player->>soulseekDifficulty';
  ...
  if(req.method==='PUT'){
   ...
   if('souls' in body)row.souls=Math.max(0,Number(body.souls)||0);
