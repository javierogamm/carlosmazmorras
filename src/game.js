...
function currentEnemyRow(){const statsBase={hp:+configEnemyHp.value||12,atk:+configEnemyAtk.value||4,armor:+configEnemyArmor.value||0,xp:+configEnemyXp.value||8},stats=normalizeEnemyCoreStats({strength:+configEnemyStrength.value||0,vitality:+configEnemyVitality.value||0,agility:+configEnemyAgility.value||0,luck:+configEnemyLuck.value||0,intelligence:+configEnemyIntelligence.value||0,wisdom:+configEnemyWisdom.value||0},configEnemyType.value);return{family:configEnemyFamilyName.value.trim()||'Sin familia',icon:window.currentConfigEnemyIconHex||'',class:configEnemyClass.value.trim(),type:configEnemyType.value,boss:configEnemyBoss.value,stats:JSON.stringify(stats),stats_base:JSON.stringify(statsBase),weapon_type:configEnemyWeaponType.value,tier:configEnemyTier.value,skill:selectedEnemySkills().join(',')}}
