const SUPABASE_TABLE='config_chest';
...
function cleanChest(body){
 const chest=body.chest_json||body;
 return {chest_name:body.chest_name??chest.name??'Cofre sin nombre',chest_json:chest};
}
function requestId(req){return req.query?.id||req.body?.id||req.body?.chest_id||null}
module.exports=async(req,res)=>{
 try{
  const {url,key}=supabaseConfig();
  if(req.method==='GET'){
   const r=await fetch(`${url}/rest/v1/${SUPABASE_TABLE}?select=id,created_at,chest_name,chest_json&order=chest_name.asc`,{headers:headers(key)});
   const data=await r.json();
   if(!r.ok)return res.status(r.status).json(data);
   return res.status(200).json(data);
  }
  if(req.method==='POST'){
   const row=cleanChest(req.body||{});
  ...
  if(req.method==='PUT'){
   ...
   if(!id)return res.status(400).json({error:'Falta id para actualizar el cofre'});
   const row=cleanChest(req.body||{});
  ...
  if(req.method==='DELETE'){
   const id=requestId(req);
   if(!id)return res.status(400).json({error:'Falta id para borrar el cofre'});
