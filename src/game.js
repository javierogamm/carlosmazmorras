...
function renderConfigItems(){const root=document.getElementById('configItemsList');if(!root)return;const equipmentRows=configItems.filter(i=>(i.item_json||i).type!=='potion');if(!equipmentRows.length){root.innerHTML='<p class="small">No hay objetos configurados.</p>';return}
...
let configDungeonWorlds=[],editingDungeonWorldId=null,dungeonPreviewWorld=null,dungeonPreviewFloor=0;
...
async function saveDungeonConfig(){const status=dungeonField('dungeonConfigStatus'),name=dungeonField('dungeonConfigName').value.trim()||'Dungeon sin nombre';status.textContent='Generando los 6 pisos y su geometría...';try{if(!configFloors.length)await fetchConfigFloors();if(!configEnemyFamilies.length)await fetchEnemyConfig();const params=dungeonConfigParams(),world=createDungeonWorldJson(name,params);world.story=params.fine.story;world.fine=params.fine;world.floors.forEach((f,i)=>f.story=params.floorPlan[i]?.story||'');const method=editingDungeonWorldId?'PATCH':'POST',r=await fetch('/api/dungeon-worlds',{method,headers:{'Content-Type':'application/json'},body:JSON.stringify({id:editingDungeonWorldId,world_name:name,world_json:world})}),data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo guardar');editingDungeonWorldId=data.id;dungeonPreviewWorld=world;dungeonPreviewFloor=0;drawDungeonGeometryPreview();await fetchConfigDungeons();status.textContent='Dungeon consolidada y vista previa actualizada.'}catch(e){status.textContent=`Error: ${e.message}`}}
