...
function normalizedConfigFloors(){const saved=configTilesets.flatMap(r=>Array.isArray(r.tileset_json?.floors)?r.tileset_json.floors:[]).filter(f=>f&&f.name);return saved.length?saved:defaultTilesetFloors}
...
function drawDungeonTile(x,y,wall,gx,gy){
 ...
 if(wall){
  const wallTiles=floorSet.wallTiles?.length?floorSet.wallTiles:[{}],wt=wallTiles[seed%wallTiles.length],rot=wt.rotatable?((seed%4)*90):0;
...
function emptyTilesetDraft(){return {tiles:[],floors:[...defaultTilesetFloors]}}
function currentTilesetDraft(){return configTilesets[0]?.tileset_json||emptyTilesetDraft()}
async function fetchConfigTilesets(){try{const r=await fetch('/api/config-tilesets');const data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudieron cargar tilesets');configTilesets=Array.isArray(data)?data:[];renderConfigTilesets()}catch(e){const st=document.getElementById('configTilesetStatus');if(st)st.textContent=`Error cargando config_tilesets: ${e.message}`}}
async function saveConfigTilesetDraft(draft){const body={nombre:'Tilesets consolidados',tileset_json:draft};const first=configTilesets[0];const r=await fetch(first?`/api/config-tilesets?id=${encodeURIComponent(first.id)}`:'/api/config-tilesets',{method:first?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo guardar tileset');await fetchConfigTilesets();return data}
function tileLabel(t,i){return `${t.name||'Tile '+(i+1)} · ${t.type==='wall'?'Muro':t.type==='door'?'Puerta':'Suelo'}`}
function renderConfigTilesets(){const draft=currentTilesetDraft(),tiles=draft.tiles||[],floors=draft.floors||defaultTilesetFloors;const list=document.getElementById('configTilesList'),fl=document.getElementById('configFloorsList');if(list)list.innerHTML=tiles.length?tiles.map((t,i)=>`<div class="configItem"><span class="tierDot" style="background:${t.color||'#263927'}"></span><div><b>${tileLabel(t,i)}</b><span class="small">Rotación: ${t.rotatable?'sí':'no'} · Icono: ${t.icon?'imagen':'colores'}</span></div></div>`).join(''):'<p class="small">Aún no hay tiles personalizados.</p>';if(fl)fl.innerHTML=floors.map((f,i)=>`<div class="configItem"><span class="tierDot" style="background:${f.floorTiles?.[0]?.color||'#263927'}"></span><div><b>${f.name}</b><span class="small">${f.floorTiles?.length||0} suelo · ${f.wallTiles?.length||0} muro · ${f.doorTiles?.length||0} puerta</span><div class="configItemActions"><button type="button" data-load-floor="${i}">Editar</button></div></div></div>`).join('');document.querySelectorAll('[data-load-floor]').forEach(btn=>btn.onclick=()=>loadConfigFloor(Number(btn.dataset.loadFloor)));renderTileSelects()}
function renderTileSelects(){const draft=currentTilesetDraft(),tiles=draft.tiles||[];for(const [id,type] of [['configFloorTiles','floor'],['configWallTiles','wall'],['configDoorTiles','door']]){const el=document.getElementById(id);if(el)el.innerHTML=tiles.map((t,i)=>({...t,i})).filter(t=>t.type===type).map(t=>`<option value="${t.i}">${t.name}</option>`).join('')}}
function selectedTiles(id){const draft=currentTilesetDraft(),tiles=draft.tiles||[];return [...(document.getElementById(id)?.selectedOptions||[])].map(o=>tiles[Number(o.value)]).filter(Boolean)}
function loadConfigFloor(i){const f=(currentTilesetDraft().floors||[])[i];if(!f)return;configFloorName.value=f.name||'';configFloorStory.value=f.story||'';configTilesetStatus.textContent=`Editando floor ${f.name}. Vuelve a guardar para consolidarlo.`}
function setupTilesetConfigMode(){setupImageIconEditor({inputId:'configTileImageInput',canvasId:'configTileCropCanvas',previewId:'configTileIconPreview',statusId:'configTileIconStatus',zoomId:'configTileCropZoom',eraserId:'configTileMagicEraserBtn',toleranceId:'configTileMagicTolerance',hexKey:'currentConfigTileIconHex',statusPrefix:'Tile'});renderConfigTilesets();addConfigTileBtn.onclick=async()=>{const draft=currentTilesetDraft();draft.tiles=draft.tiles||[];draft.tiles.push({name:configTileName.value.trim()||'Tile sin nombre',type:configTileType.value,color:configTileColor.value,alt:configTileAlt.value,top:configTileAlt.value,accent:configTileAccent.value,rotatable:configTileType.value==='wall'&&configTileRotatable.checked,icon:window.currentConfigTileIconHex||''});configTilesetStatus.textContent='Guardando tile...';try{await saveConfigTilesetDraft(draft);configTilesetStatus.textContent='Tile guardado.'}catch(e){configTilesetStatus.textContent=e.message}};saveConfigFloorBtn.onclick=async()=>{const draft=currentTilesetDraft();draft.floors=draft.floors||[];const f={id:configFloorName.value.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-'),name:configFloorName.value.trim()||'Floor sin nombre',story:configFloorStory.value.trim(),floorTiles:selectedTiles('configFloorTiles'),wallTiles:selectedTiles('configWallTiles'),doorTiles:selectedTiles('configDoorTiles')};const idx=draft.floors.findIndex(x=>x.name===f.name);if(idx>=0)draft.floors[idx]=f;else draft.floors.push(f);configTilesetStatus.textContent='Guardando floor...';try{await saveConfigTilesetDraft(draft);configTilesetStatus.textContent='Floor guardado.'}catch(e){configTilesetStatus.textContent=e.message}};exportConfigTilesetBtn.onclick=()=>{const blob=new Blob([JSON.stringify(currentTilesetDraft(),null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='config-tilesets.json';a.click();URL.revokeObjectURL(a.href)}}

function setupConfigTabs(){document.querySelectorAll('[data-config-tab]').forEach(btn=>btn.onclick=()=>{const tab=btn.dataset.configTab;document.querySelectorAll('[data-config-tab]').forEach(b=>b.classList.toggle('active',b===btn));configTabItems.classList.toggle('hidden',tab!=='items');configTabClasses.classList.toggle('hidden',tab!=='classes');configTabTilesets.classList.toggle('hidden',tab!=='tilesets')})}
