...
function makeConfiguredLoot(level){if(!configItems.length)return null;const row=pick(configItems),raw=row.item_json||row,item={...raw};item.id=crypto.randomUUID();item.name=item.name||row.nombre||'Objeto configurado';item.slot=item.slot||row.slot||'trinket1';item.rarity=item.rarity||row.tier||'common';item.label=item.label||tierDefs[item.rarity]?.label||item.rarity;item.itemLevel=Number(item.itemLevel||row.ilvl||level||1);item.score=Number(item.score||item.itemLevel*8);item.icon=item.icon||row.icon||'';item.damageDice=item.slot==='weapon'?(item.damageDice||row.damageDice||'1d6'):null;item.affixes=Array.isArray(item.affixes)?item.affixes:parseConfigStats(row.stats||item.stats);item.passives=item.passives||[];item.effects=item.effects||[];item.desc=item.desc||`Configurado · Nivel ${item.itemLevel} · Poder ${item.score}`;item.flavor=item.flavor||'Objeto creado en modo configuración.';return item}
...
function currentConfigItemJson(){const slot=configSlot.value;return {name:configNombre.value.trim()||'Objeto sin nombre',slot,rarity:configTier.value,label:tierDefs[configTier.value]?.label||configTier.value,itemLevel:Number(configIlvl.value||1),score:Number(configIlvl.value||1)*8,icon:window.currentConfigIconHex||'',damageDice:slot==='weapon'?configDamageDice.value:null,stats:configStats.value,affixes:parseConfigStats(configStats.value),passives:[],effects:[],desc:`Configurado · ${configStats.value||'sin stats'}`}}
