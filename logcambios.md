## v0.55.2 - Corrige el turno multijugador que se quedaba colgado en el jugador 2
- Diagnóstico: `mpApplyLiveTurn()` (turno recibido por Realtime) y `mpApplyRemoteState()` (turno recibido por el sondeo/checkpoint de BBDD) marcaban el `seq`/`rev` recibido como ya aplicado **antes** de terminar de fusionar el estado (posiciones, enemigos, puertas, cofres, `recomputeDerived()`, `updateUI()`, `draw()`...). Si cualquier paso de esa fusión lanzaba una excepción -con datos de red inesperados, por ejemplo-, el cambio de turno (`activePlayerIndex`/`myTurn`) nunca llegaba a aplicarse, pero el contador ya quedaba al día: el siguiente `turn_commit`/checkpoint del otro jugador se descartaba en silencio por considerarse un duplicado ya visto, dejando al jugador 2 esperando su turno para siempre sin ningún error visible.
- Ambas funciones se reestructuran para que el cambio de turno (`activePlayerIndex` + `mpSetMyTurn()`) sea incondicional: se calcula antes de la fusión del resto del estado, y esa fusión (secundaria: enemigos/puertas/cofres/trampas/eventos/replay visual) queda envuelta en su propio `try/catch` que solo registra el error (`mpReportError`) sin poder impedir que el turno cambie de manos. Añadida además una red de seguridad equivalente en `mpOnRemoteTurn` para que el ACK siempre se envíe.
- Bump del cache-busting de `src/game.js` (`?v=`).
- Versión 0.55.2.
