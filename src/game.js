...
function makeConfiguredLoot(level){if(!configItems.length)return null;const row=pick(configItems),raw=row.item_json||row,item={...raw};item.id=crypto.randomUUID();item.name=item.name||row.nombre||'Objeto configurado';item.slot=item.slot||row.slot||'trinket1';item.rarity=item.rarity||row.tier||'common';item.label=item.label||tierDefs[item.rarity]?.label||item.rarity;item.itemLevel=Number(item.itemLevel||row.ilvl||level||1);item.score=Number(item.score||item.itemLevel*8);item.icon=item.icon||row.icon||'';item.damageDice=item.slot==='weapon'?(item.damageDice||row.damageDice||'1d6'):null;item.skillIds=Array.isArray(item.skillIds)?item.skillIds:[];item.affixes=Array.isArray(item.affixes)?item.affixes:parseConfigStats(row.stats||item.stats);item.passives=item.passives||[];item.effects=item.effects||[];item.desc=item.desc||`Configurado · Nivel ${item.itemLevel} · Poder ${item.score}`;item.flavor=item.flavor||'Objeto creado en modo configuración.';return item}
...
function classIconForId(id){const def=classDefs[id];const wanted=normalizeClassName(def?.name||id);const row=configClasses.find(c=>normalizeClassName(c.nombre)===wanted||String(c.class_json?.classId||'')===id);return row?.class_json?.icon||row?.icon||''}
function classIdForConfigRow(row){const wanted=normalizeClassName(row?.nombre);return Object.entries(classDefs).find(([id,c])=>normalizeClassName(c.name)===wanted||id===row?.class_json?.classId)?.[0]||selectedClass}
...
function currentConfigItemJson(){const slot=configSlot.value,skillIds=selectedConfigSkillIds();return {name:configNombre.value.trim()||'Objeto sin nombre',slot,rarity:configTier.value,label:tierDefs[configTier.value]?.label||configTier.value,itemLevel:Number(configIlvl.value||1),score:Number(configIlvl.value||1)*8,icon:window.currentConfigIconHex||'',damageDice:slot==='weapon'?configDamageDice.value:null,skillIds,stats:configStats.value,affixes:parseConfigStats(configStats.value),passives:[],effects:[],desc:`Configurado · ${configStats.value||'sin stats'}${skillIds.length?` · Habilidades: ${skillIds.map(id=>skillDefs[id]?.name||id).join(', ')}`:''}`}}
...
function currentConfigClassJson(){const row=configClasses.find(c=>String(c.id)===String(configClassSelect.value));return {classId:classIdForConfigRow(row),name:row?.nombre||'Clase sin nombre',icon:window.currentConfigClassIconHex||'',stats:row?.stats||'',skills:row?.skills||''}}
async function updateConfigClass(id,item){const r=await fetch(`/api/config-class?id=${encodeURIComponent(id)}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...item,id,class_json:item})});const data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo actualizar la clase');await fetchConfigClasses();return data}
function renderConfigClasses(){const sel=document.getElementById('configClassSelect'),list=document.getElementById('configClassesList');if(!sel)return;sel.innerHTML=configClasses.map(c=>`<option value="${c.id}">${c.nombre||'Clase sin nombre'} (#${c.id})</option>`).join('');if(list)list.innerHTML=configClasses.length?configClasses.map(c=>`<div class="configItem"><span class="tierDot" style="background:#8c72e8"></span><div><b>${c.nombre||'Sin nombre'}</b><span class="small">#${c.id} · ${c.icon?'Icono subido':'Pixels por defecto'}</span></div></div>`).join(''):'<p class="small">No hay clases en config_class.</p>';if(configClasses.length&&!sel.value)sel.value=configClasses[0].id;loadSelectedConfigClass()}
function loadSelectedConfigClass(){const row=configClasses.find(c=>String(c.id)===String(configClassSelect.value));configClassName.value=row?.nombre||'';window.currentConfigClassIconHex=row?.class_json?.icon||row?.icon||'';renderConfigIconPreview(window.currentConfigClassIconHex,'configClassIconPreview','configClassIconStatus');configClassIconStatus.textContent=window.currentConfigClassIconHex?'Icono cargado para esta clase.':'Sin icono: usará los pixels por defecto.'}

function renderConfigItems(){const root=document.getElementById('configItemsList');if(!root)return;root.innerHTML=configItems.length?configItems.map(i=>{const item=i.item_json||i,skills=Array.isArray(item.skillIds)?item.skillIds:[];return `<div class="configItem"><span class="tierDot" style="background:${tierColor(i.tier)}"></span><div><b>${i.nombre||'Sin nombre'}</b><span class="small">${slotNames[i.slot]||i.slot} · ${tierDefs[i.tier]?.label||i.tier} · iLvl ${i.ilvl||1}${skills.length?` · ${skillNames(skills)}`:''}</span><div class="configItemActions"><button type="button" data-config-edit="${i.id}">Editar</button><button type="button" data-config-duplicate="${i.id}">Duplicar</button><button type="button" data-config-delete="${i.id}">Borrar</button></div></div></div>`}).join(''):'<p class="small">No hay objetos configurados.</p>';root.querySelectorAll('[data-config-edit]').forEach(b=>b.onclick=()=>loadConfigItemForEdit(b.dataset.configEdit));root.querySelectorAll('[data-config-duplicate]').forEach(b=>b.onclick=()=>duplicateConfigItem(b.dataset.configDuplicate));root.querySelectorAll('[data-config-delete]').forEach(b=>b.onclick=()=>removeConfigItem(b.dataset.configDelete))}
...
function setupConfigTabs(){document.querySelectorAll('[data-config-tab]').forEach(btn=>btn.onclick=()=>{const tab=btn.dataset.configTab;document.querySelectorAll('[data-config-tab]').forEach(b=>b.classList.toggle('active',b===btn));configTabItems.classList.toggle('hidden',tab!=='items');configTabClasses.classList.toggle('hidden',tab!=='classes')})}

function setupClassConfigMode(){const imgInput=document.getElementById('configClassImageInput'),preview=document.getElementById('configClassIconPreview');if(!imgInput)return;configClassSelect.onchange=loadSelectedConfigClass;imgInput.onchange=()=>{const f=imgInput.files?.[0];if(!f)return;const img=new Image();img.onload=()=>{const out=document.createElement('canvas');out.width=out.height=50;const o=out.getContext('2d');o.imageSmoothingEnabled=false;const side=Math.min(img.naturalWidth,img.naturalHeight),sx=(img.naturalWidth-side)/2,sy=(img.naturalHeight-side)/2;o.drawImage(img,sx,sy,side,side,0,0,50,50);preview.getContext('2d').clearRect(0,0,50,50);preview.getContext('2d').drawImage(out,0,0);fetch(out.toDataURL('image/png')).then(r=>r.arrayBuffer()).then(buf=>{window.currentConfigClassIconHex=[...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join('');configClassIconStatus.textContent='Icono de clase preparado (50x50).'});};img.src=URL.createObjectURL(f)};saveConfigClassBtn.onclick=async()=>{if(!configClassSelect.value){configClassStatus.textContent='Selecciona una clase existente.';return}configClassStatus.textContent='Actualizando clase...';try{await updateConfigClass(configClassSelect.value,currentConfigClassJson());configClassStatus.textContent='Clase actualizada.'}catch(e){configClassStatus.textContent=e.message}}}
function setupConfigMode(){renderConfigStatsHelp();renderConfigSkillSelect();const slotSel=document.getElementById('configSlot');if(slotSel&&!slotSel.options.length)slotSel.innerHTML=slots.map(s=>`<option value="${s}">${slotNames[s]}</option>`).join('');const toggleDamageDice=()=>{configDamageDiceWrap.classList.toggle('hidden',configSlot.value!=='weapon')};slotSel.onchange=toggleDamageDice;toggleDamageDice();const imgInput=document.getElementById('configImageInput'),crop=document.getElementById('configCropCanvas'),preview=document.getElementById('configIconPreview');let img=null,rect=null,drag=null;function clampRect(r){const size=Math.max(1,Math.min(Math.round(r.w),crop.width,crop.height));return{x:Math.max(0,Math.min(Math.round(r.x),Math.max(0,crop.width-size))),y:Math.max(0,Math.min(Math.round(r.y),Math.max(0,crop.height-size))),w:size,h:size}}function pointerPos(e){const b=crop.getBoundingClientRect();return{x:(e.clientX-b.left)*crop.width/b.width,y:(e.clientY-b.top)*crop.height/b.height}}function inRect(p,r){return r&&p.x>=r.x&&p.x<=r.x+r.w&&p.y>=r.y&&p.y<=r.y+r.h}function drawCrop(){const c=crop.getContext('2d');c.imageSmoothingEnabled=false;c.clearRect(0,0,crop.width,crop.height);c.fillStyle='#100b16';c.fillRect(0,0,crop.width,crop.height);if(img)c.drawImage(img,0,0);if(rect){c.save();c.fillStyle='#0008';c.fillRect(0,0,crop.width,rect.y);c.fillRect(0,rect.y+rect.h,crop.width,crop.height-rect.y-rect.h);c.fillRect(0,rect.y,rect.x,rect.h);c.fillRect(rect.x+rect.w,rect.y,crop.width-rect.x-rect.w,rect.h);c.strokeStyle='#ffd68b';c.lineWidth=2;c.strokeRect(rect.x+.5,rect.y+.5,rect.w,rect.h);c.fillStyle='#ffd68b';c.fillRect(rect.x+rect.w-5,rect.y+rect.h-5,5,5);c.restore()}}function saveIcon(){if(!img||!rect)return;const out=document.createElement('canvas');out.width=out.height=50;const o=out.getContext('2d');o.imageSmoothingEnabled=false;o.clearRect(0,0,50,50);o.drawImage(img,rect.x,rect.y,rect.w,rect.h,0,0,50,50);preview.getContext('2d').clearRect(0,0,50,50);preview.getContext('2d').drawImage(out,0,0);fetch(out.toDataURL('image/png')).then(r=>r.arrayBuffer()).then(buf=>{window.currentConfigIconHex=[...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join('');configIconStatus.textContent=`Icono 50x50 desde x:${rect.x}, y:${rect.y}, lado:${rect.w}`})}function updateDrag(e){if(!img||!drag)return;const p=pointerPos(e);if(drag.mode==='move')rect=clampRect({x:p.x-drag.dx,y:p.y-drag.dy,w:drag.start.w,h:drag.start.h});else{const size=Math.max(1,Math.min(Math.abs(p.x-drag.origin.x),Math.abs(p.y-drag.origin.y)));rect=clampRect({x:p.x<drag.origin.x?drag.origin.x-size:drag.origin.x,y:p.y<drag.origin.y?drag.origin.y-size:drag.origin.y,w:size,h:size})}drawCrop();saveIcon()}imgInput.onchange=()=>{const f=imgInput.files?.[0];if(!f)return;img=new Image();img.onload=()=>{crop.width=img.naturalWidth;crop.height=img.naturalHeight;rect=clampRect({x:0,y:0,w:Math.min(50,crop.width,crop.height),h:Math.min(50,crop.width,crop.height)});drawCrop();saveIcon();configIconStatus.textContent=`Imagen original ${crop.width}x${crop.height}. Dibuja o mueve el cuadrado.`};img.src=URL.createObjectURL(f)};crop.onpointerdown=e=>{if(!img)return;crop.setPointerCapture?.(e.pointerId);const p=pointerPos(e);if(inRect(p,rect))drag={mode:'move',start:{...rect},dx:p.x-rect.x,dy:p.y-rect.y};else{drag={mode:'draw',origin:p};rect=clampRect({x:p.x,y:p.y,w:1,h:1})}drawCrop()};crop.onpointermove=e=>{if(e.buttons)updateDrag(e)};crop.onpointerup=e=>{crop.releasePointerCapture?.(e.pointerId);updateDrag(e);drag=null;saveIcon()};saveConfigItemBtn.onclick=async()=>{configStatus.textContent=window.editingConfigItemId?'Actualizando...':'Guardando...';try{const item=currentConfigItemJson();if(window.editingConfigItemId)await updateConfigItem(window.editingConfigItemId,item);else await saveConfigItems([{...item,nombre:configNombre.value,slot:configSlot.value,tier:configTier.value,ilvl:configIlvl.value,item_json:item}]);configStatus.textContent=window.editingConfigItemId?'Objeto actualizado.':'Objeto guardado.'}catch(e){configStatus.textContent=e.message}};newConfigItemBtn.onclick=resetConfigForm;exportConfigJsonBtn.onclick=()=>{const blob=new Blob([JSON.stringify(currentConfigItemJson(),null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='config-item.json';a.click();URL.revokeObjectURL(a.href)};importConfigJsonInput.onchange=async()=>{const files=[...importConfigJsonInput.files];configStatus.textContent='Importando...';try{const items=[];for(const f of files){const parsed=JSON.parse(await f.text());items.push(...(Array.isArray(parsed)?parsed:[parsed]))}await saveConfigItems(items.map(x=>({...x,item_json:x})));configStatus.textContent=`Importados ${items.length} objeto(s).`}catch(e){configStatus.textContent=e.message}}}
