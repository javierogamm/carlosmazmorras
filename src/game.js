...
const configWeaponTypes=['Espadas','Dagas','Sables','Hachas','Mazas','Martillos','Lanzas','Bastones','Arcos','Ballestas','Pistolas','Rifles','Escopetas','Armas pesadas','Guanteletes','Látigos','Drones','Granadas','Artefactos'];
const configWeaponTypeCategories={Espadas:'Armas blancas steampunk básicas',Dagas:'Armas blancas steampunk básicas',Sables:'Armas de latón refinadas',Hachas:'Armas pesadas steampunk',Mazas:'Armas pesadas steampunk',Martillos:'Armas pesadas steampunk',Lanzas:'Espadas eléctricas iniciales',Bastones:'Armas criogénicas',Arcos:'Armas a distancia mecánicas',Ballestas:'Armas a distancia mecánicas',Pistolas:'Armas de fuego ciberpunk',Rifles:'Armamento steampunk avanzado',Escopetas:'Armas de pólvora industrial','Armas pesadas':'Artillería steampunk',Guanteletes:'Armas ciberpunk pesadas',Látigos:'Armas térmicas',Drones:'Armas ciberpunk pesadas',Granadas:'Armas tóxicas y biotecnológicas',Artefactos:'Artefactos de energía'};
...
function start(){
 ...
 game={floor:1,themeIndex:0,turn:0,dungeonWorldId:selectedDungeonWorld?.id||null,dungeonWorldName:selectedDungeonWorld?.world_name||null,inventory:[],achievements:{},bossesKilled:0,chestsOpened:0,player:{name:nameInput.value||'Sin nombre',race,cls:selectedClass,className:cls.name,classIcon:classIconForId(selectedClass),level:1,xp:0,nextXp:xpNeededForLevel(1),hp:maxHp,maxHp,stamina:maxStamina,maxStamina,mana:maxMana,maxMana,baseDamage:2+stats.strength,baseArmor:4+Math.floor(stats.vitality/2),gold:0,keys:0,vision:4+Math.floor(stats.agility/4),shield:0,stats,equipment,knownSkills:[],skillProgress:{},skillChoicesAwarded:{},equippedSkills:[null,null,null,null],cooldowns:{},debuff:0}};
...
function currentConfigItemJson(){const slot=configSlot.value,skillIds=selectedConfigSkillIds(),weaponType=slot==='weapon'?(configWeaponCategory.value||configWeaponTypes[0]):null,weaponCategory=weaponType?(configWeaponTypeCategories[weaponType]||weaponCategories[0]):null,weaponIconRow=weaponCategory?weaponRowForCategory(weaponCategory):null,weaponIconCol=weaponCategory?weaponPowerColumn(Number(configIlvl.value||1),configTier.value,Number(configIlvl.value||1)*8):null;return {name:configNombre.value.trim()||'Objeto sin nombre',slot,rarity:configTier.value,label:tierDefs[configTier.value]?.label||configTier.value,itemLevel:Number(configIlvl.value||1),score:Number(configIlvl.value||1)*8,icon:window.currentConfigIconHex||'',damageDice:slot==='weapon'?configDamageDice.value:null,weaponType,weaponCategory,weaponIconRow,weaponIconCol,weaponIconPath:weaponCategory?weaponIconPath(weaponIconRow,weaponIconCol):null,defenseStat:weaponCategory?(weaponCategoryStats[weaponCategory]||'strength'):null,skillIds,stats:configStats.value,affixes:parseConfigStats(configStats.value),passives:[],effects:[],desc:`Configurado · ${configStats.value||'sin stats'}${skillIds.length?` · Habilidades: ${skillIds.map(id=>skillDefs[id]?.name||id).join(', ')}`:''}`}}
...
function renderConfigSkillSelect(){const sel=document.getElementById('configSkills');if(!sel)return;sel.innerHTML=Object.entries(skillDefs).sort((a,b)=>(a[1].name||a[0]).localeCompare(b[1].name||b[0],'es')).map(([id,d])=>`<option value="${id}">${d.icon||'•'} ${d.name} · ${tierDefs[d.rarity]?.label||d.rarity||'Común'}</option>`).join('')}
...
async function fetchConfigFloors(){try{const r=await fetch('/api/config-floor');const data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudieron cargar floors');configFloors=Array.isArray(data)?data:[];renderConfigTilesets()}catch(e){const st=document.getElementById('configTilesetStatus');if(st)st.textContent=`Error cargando config_floor: ${e.message}`}}
async function saveConfigFloorRow(floor){const id=window.editingConfigFloorId;const r=await fetch(id?`/api/config-floor?id=${encodeURIComponent(id)}`:'/api/config-floor',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({floor_name:floor.name,floor_json:floor})});const data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo guardar floor');await fetchConfigFloors();return data}
...
function parseEnemyStatsBase(v){if(!v)return{hp:12,atk:4,armor:0,xp:8};if(typeof v==='object')return{hp:+v.hp||12,atk:+(v.atk??v.damage)||4,armor:+v.armor||0,xp:+v.xp||8};try{return parseEnemyStatsBase(JSON.parse(v))}catch(e){return Object.fromEntries(String(v).split(',').map(x=>x.trim().split(':')).filter(x=>x[0]).map(([k,v])=>[k,Number(v)||0]))}}
function normalizeEnemyDetail(row){const stats=parseEnemyStatsBase(row.stats_base),boss=String(row.boss||'no').toLowerCase().startsWith('s');return{id:row.id,family:row.family||'Sin familia',name:row.class||row.type||'Enemigo',class:row.class||'',type:row.type||'warrior',icon:row.icon||'',boss,tier:String(row.tier||'i').toLowerCase(),weaponType:row.weapon_type||'',statsBase:{hp:stats.hp||12,atk:stats.atk||stats.damage||4,armor:stats.armor||0,xp:stats.xp||8},skillIds:String(row.skill||'').split(/[,;\s]+/).map(x=>x.trim()).filter(Boolean)}}
...
function setEnemySkills(ids){[...configEnemySkills.options].forEach(o=>o.selected=(ids||[]).includes(o.value))}
function currentEnemyRow(){const stats={hp:+configEnemyHp.value||12,atk:+configEnemyAtk.value||4,armor:+configEnemyArmor.value||0,xp:+configEnemyXp.value||8};return{family:configEnemyFamilyName.value.trim()||'Sin familia',icon:window.currentConfigEnemyIconHex||'',class:configEnemyClass.value.trim(),type:configEnemyType.value,boss:configEnemyBoss.value,stats_base:JSON.stringify(stats),weapon_type:configEnemyWeaponType.value,tier:configEnemyTier.value,skill:selectedEnemySkills().join(',')}}
function resetEnemyForm(){window.editingConfigEnemyId=null;configEnemyClass.value='';configEnemyBoss.value='no';configEnemyTier.value='i';configEnemyHp.value=12;configEnemyAtk.value=4;configEnemyArmor.value=0;configEnemyXp.value=8;setEnemySkills([]);window.currentConfigEnemyIconHex='';renderConfigIconPreview('','configEnemyIconPreview','configEnemyIconStatus');configEnemyStatus.textContent='Nuevo enemigo.'}
function loadEnemyForEdit(id){const row=configEnemyDetails.find(e=>String(e.id)===String(id));if(!row)return;const e=normalizeEnemyDetail(row);window.editingConfigEnemyId=row.id;configEnemyFamilyName.value=e.family;configEnemyClass.value=e.class;configEnemyType.value=e.type;configEnemyBoss.value=e.boss?'si':'no';configEnemyTier.value=e.tier;configEnemyWeaponType.value=e.weaponType;configEnemyHp.value=e.statsBase.hp;configEnemyAtk.value=e.statsBase.atk;configEnemyArmor.value=e.statsBase.armor;configEnemyXp.value=e.statsBase.xp;setEnemySkills(e.skillIds);window.currentConfigEnemyIconHex=e.icon;renderConfigIconPreview(e.icon,'configEnemyIconPreview','configEnemyIconStatus');configEnemyIconStatus.textContent=e.icon?'Icono cargado desde enemigo guardado.':'Sin icono';configEnemyStatus.textContent=`Editando enemigo #${row.id}.`}
function renderEnemyConfig(){renderEnemySkillSelect();if(configEnemyWeaponType&&!configEnemyWeaponType.options.length)configEnemyWeaponType.innerHTML=configWeaponTypes.map(c=>`<option value="${c}">${c}</option>`).join('');if(configEnemyFamiliesList)configEnemyFamiliesList.innerHTML=(configEnemyFamilies.length?configEnemyFamilies.map(r=>`<div class="configItem"><span class="tierDot" style="background:#9b65d8"></span><div><b>${r.family_name}</b><span class="small">${(r.family_json?.enemies||[]).length} enemigos en JSON</span><div class="configItemActions"><button type="button" data-load-family="${r.id}">Editar familia</button></div></div></div>`).join(''):'<p class="small">No hay familias consolidadas.</p>');if(configEnemiesList)configEnemiesList.innerHTML=(configEnemyDetails.length?configEnemyDetails.map(r=>{const e=normalizeEnemyDetail(r);return`<div class="configItem"><span class="tierDot" style="background:${e.boss?'#ffb746':'#75e39d'}"></span><div><b>${e.class||e.type}</b><span class="small">${e.family} · ${e.type} · tier ${e.tier.toUpperCase()} · boss ${e.boss?'sí':'no'}</span><div class="configItemActions"><button type="button" data-edit-enemy="${r.id}">Editar</button></div></div></div>`}).join(''):'<p class="small">No hay enemigos individuales.</p>');document.querySelectorAll('[data-edit-enemy]').forEach(b=>b.onclick=()=>loadEnemyForEdit(b.dataset.editEnemy));document.querySelectorAll('[data-load-family]').forEach(b=>b.onclick=()=>{const r=configEnemyFamilies.find(x=>String(x.id)===b.dataset.loadFamily);if(r){configEnemyFamilyName.value=r.family_name;configEnemyStatus.textContent=`Familia ${r.family_name} cargada para editar enemigos.`}})}
function setupEnemyConfigMode(){setupImageIconEditor({inputId:'configEnemyImageInput',canvasId:'configEnemyCropCanvas',previewId:'configEnemyIconPreview',statusId:'configEnemyIconStatus',zoomId:'configEnemyCropZoom',eraserId:'configEnemyMagicEraserBtn',toleranceId:'configEnemyMagicTolerance',hexKey:'currentConfigEnemyIconHex',statusPrefix:'Icono enemigo'});renderEnemyConfig();saveConfigEnemyBtn.onclick=async()=>{configEnemyStatus.textContent='Guardando enemigo...';try{const row=currentEnemyRow(),id=window.editingConfigEnemyId,r=await fetch(id?`/api/enemy-detail?id=${encodeURIComponent(id)}`:'/api/enemy-detail',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(row)}),data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo guardar enemigo');await fetchEnemyConfig();configEnemyStatus.textContent='Enemigo guardado.'}catch(e){configEnemyStatus.textContent=e.message}};newConfigEnemyBtn.onclick=resetEnemyForm;saveEnemyFamilyBtn.onclick=async()=>{try{const name=configEnemyFamilyName.value.trim()||'Sin familia',json=familyJsonFromDetails(name),existing=configEnemyFamilies.find(r=>r.family_name===name),r=await fetch(existing?`/api/enemy-family?id=${existing.id}`:'/api/enemy-family',{method:existing?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({family_name:name,family_json:json})}),data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo consolidar');await fetchEnemyConfig();configEnemyStatus.textContent='Familia consolidada en enemy_family.'}catch(e){configEnemyStatus.textContent=e.message}};exportEnemyFamilyBtn.onclick=()=>{const blob=new Blob([JSON.stringify(familyJsonFromDetails(configEnemyFamilyName.value.trim()||'Sin familia'),null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='enemy-family.json';a.click();URL.revokeObjectURL(a.href)};importEnemyFamilyInput.onchange=async()=>{try{for(const f of [...importEnemyFamilyInput.files]){const j=JSON.parse(await f.text());await fetch('/api/enemy-family',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({family_name:j.name||j.family_name||'Familia importada',family_json:j})})}await fetchEnemyConfig();configEnemyStatus.textContent='Familia(s) importadas.'}catch(e){configEnemyStatus.textContent=e.message}}}
