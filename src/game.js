...
function describePotionEffect(item){const e=item?.potionEffect||item?.effect||{},type=item?.potionEffectType||item?.potionId||item?.kind;if(item?.type!=='potion')return '';const parts=[];if(e.healPct)parts.push(`Cura ${Math.round(e.healPct*100)}% vida`);if(e.regenPct)parts.push(`Regenera ${Math.round(e.regenPct*100)}% vida/turno`);if(e.staminaPct)parts.push(`Recupera ${Math.round(e.staminaPct*100)}% stamina`);if(e.manaPct)parts.push(`Recupera ${Math.round(e.manaPct*100)}% maná`);const stats=e.stats||{};const statText=Object.entries(stats).filter(([,v])=>Number(v)).map(([k,v])=>`${v>0?'+':''}${v} ${DEFENSE_STAT_LABELS[k]||k}`).join(', ');if(statText)parts.push(statText);if(e.skillId)parts.push(`Aprende ${skillDefs[e.skillId]?.name||e.skillId}`);if(e.invulnerable)parts.push('Invulnerabilidad');if(e.invisible)parts.push('Invisibilidad');if(type==='teleportSafe')parts.push('Teletransporte a sala segura');if(type==='teleportStairs')parts.push('Teletransporte a escalera');if(item.duration)parts.push(`${item.duration} turnos`);return parts.join(' · ')}
...
function recomputeDerived(){
 ...
 for(const pot of p.activePotions||[]){const e=pot.effect||{},stats=e.stats||{};for(const [k,v] of Object.entries(stats))if(k in allStats)allStats[k]+=Number(v)||0;if(e.armorMult)d.armor=Math.round(d.armor*(1+e.armorMult));if(e.vision)d.vision=(d.vision||p.vision||0)+(Number(e.vision)||0);if(e.staminaRegen)d.staminaRegen+=Number(e.staminaRegen)||0;if(e.manaRegen)d.manaRegen+=Number(e.manaRegen)||0}
...
function usePotion(id){
 ...
 if(item.kind==='instant'){
  ...
  if(eff.healPct)healEntity(p,Math.round(p.maxHp*eff.healPct));
  if(eff.manaPct)p.mana=Math.min(p.maxMana,p.mana+Math.round(p.maxMana*eff.manaPct));
  if(eff.staminaPct)p.stamina=Math.min(p.maxStamina,p.stamina+Math.round(p.maxStamina*eff.staminaPct));
...
function tickPotionEffects(){
 ...
 for(const e of p.activePotions){const eff=e.effect||{};if(eff.regenPct)healEntity(p,Math.round(p.maxHp*eff.regenPct));e.turns--}p.activePotions=p.activePotions.filter(e=>e.turns>0);recomputeDerived()
}
...
function potionEffectFromConfig(){
 const type=configPotionEffect?.value||'heal',power=(Number(configPotionPower?.value)||0)/100,turns=Number(configPotionTurns?.value)||0,statsObj=Object.fromEntries(parseConfigStats(configStats.value).filter(a=>potionStatKeys.includes(a.key)).map(a=>[a.key,a.value])),fallbackStat=configPotionStat?.value||'strength';
 if(!Object.keys(statsObj).length&&(type==='temporaryStats'||type==='permanentStats'))statsObj[fallbackStat]=Math.max(1,Math.round(Number(configPotionPower?.value)||1));
 const effect={};
 if(type==='heal')effect.healPct=power||.25;if(type==='regen')effect.regenPct=power||.08;if(type==='stamina')effect.staminaPct=power||.5;if(type==='mana')effect.manaPct=power||.45;
 if(type==='temporaryStats'||type==='permanentStats')effect.stats=statsObj;if(type==='learnSkill')effect.skillId=configPotionSkill?.value||selectedConfigSkillIds()[0]||'';
...
function currentConfigItemJson(){
 const itemType=configItemType?.value||'equipment',skillIds=selectedConfigSkillIds(),ilvl=Number(configIlvl.value||1),rarity=configTier.value;
 if(itemType==='potion'){
