...
function buildPersistenceState(){if(!game)return null;return{version:APP_VERSION,multiplayer:!!multiplayerMode,turn:game.turn,floor:game.floor,dungeonWorldId:game.dungeonWorldId,playerId:selectedCharacter?.id||null,players:selectedCharacter?[String(selectedCharacter.id)]:[],player:game.player,map:game.map,enemies:game.enemies,loot:game.loot,chests:game.chests,doors:game.doors,stairs:game.stairs,safeRooms:game.safeRooms,position:game.player?{x:game.player.x,y:game.player.y}:null,updatedAt:new Date().toISOString()}}
function calculatePlayerScore(p){const eq=Object.values(p?.equipment||{}).filter(Boolean),ilvl=eq.reduce((a,i)=>a+(+i.itemLevel||+i.ilvl||0),0);return Math.round(((+p?.level||1)*100)+((+game?.floor||1)*75)+ilvl*12+((p?.inventory||[]).length*3)+((+p?.gold||0)/10)+((p?.knownSkills||[]).length*8))}
async function persistTurnState(){if(!game?.player||!window.currentUser)return;const body={nombre:window.currentUser.nombre,pj_name:game.player.name,pj_json:{...game.player,floor:game.floor,bestFloor:Math.max(game.floor,game.player.bestFloor||1),status:game.over?'dead':'alive'},pj_status:game.over?'dead':'alive',pj_score:calculatePlayerScore(game.player),last_use:new Date().toISOString()};try{if(selectedCharacter?.id){selectedCharacter=await fetch('/api/user-pj?id='+encodeURIComponent(selectedCharacter.id),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...body,id:selectedCharacter.id})}).then(r=>r.json())}else{selectedCharacter=await fetch('/api/user-pj',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(r=>r.json())}const state=buildPersistenceState();if(state&&selectedDungeonWorld){const payload={dungeon_world_id:selectedDungeonWorld.id,players_ID:state.players.join(','),dungeon_status:state};if(selectedDungeonSession?.id)selectedDungeonSession=await fetch('/api/dungeon-status?id='+selectedDungeonSession.id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...payload,id:selectedDungeonSession.id})}).then(r=>r.json());else selectedDungeonSession=await fetch('/api/dungeon-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(r=>r.json())}}catch(e){console.warn('persistencia fallida',e)}}
function start(){
...
function playerFinished(){persistTurnState();
...
function characterCard(r){return '<button class="worldCard characterCardBtn" data-pj="'+r.id+'"><b>'+ (r.pj_name||'Sin nombre')+'</b><span>'+r.nombre+' · '+(r.pj_status||'?')+' · score '+(r.pj_score||0)+'</span><small>'+(r.pj_json?.className||r.pj_json?.cls||'Clase ?')+' · '+(r.pj_json?.raceName||r.pj_json?.race||'Raza ?')+' · nivel '+(r.pj_json?.level||1)+'</small></button>'}
function newCharacterFlow(){selectedCharacter=null;createCharacterOnly=true;document.getElementById('characterHubOverlay')?.classList.add('hidden');document.getElementById('multiplayerOverlay')?.classList.add('hidden');startOverlay.classList.remove('hidden')}
function openWorldCreator(){document.getElementById('characterHubOverlay')?.classList.add('hidden');document.getElementById('multiplayerOverlay')?.classList.add('hidden');dungeonOverlay.classList.remove('hidden');fetchDungeonWorlds()}
function showCharacterHub(){multiplayerMode=false;let ov=document.getElementById('characterHubOverlay');if(!ov){ov=document.createElement('div');ov.id='characterHubOverlay';ov.className='overlay';ov.innerHTML='<div class="modal hubModal"><h2>PERSONAJES</h2><p class="small">Elige cómo quieres jugar: crea o selecciona un personaje y después crea o continúa una sesión de mundo.</p><div class="hubActions"><button class="hubAction primary" id="continuePjBtn"><b>Continuar</b><span>Último personaje usado</span></button><button class="hubAction" id="selectPjBtn"><b>Seleccionar personaje</b><span>Solo personajes vivos</span></button><button class="hubAction" id="newPjBtn"><b>Nuevo personaje</b><span>Raza, clase y nombre</span></button><button class="hubAction" id="scoreBtn"><b>Puntuaciones</b><span>Ranking global</span></button><button class="hubAction" id="createWorldHubBtn"><b>Crear mundo</b><span>Dificultad, floors y familias</span></button><button class="hubAction" id="multiBtn"><b>Multiplayer</b><span>Sala con usuarios activos</span></button><button class="hubAction adminOnly" id="adminConfigBtn"><b>Configurar</b><span>Panel admin</span></button></div><div id="characterHubList" class="worldList hubList"></div></div>';document.body.appendChild(ov)}const $=id=>ov.querySelector('#'+id);ov.classList.remove('hidden');$('adminConfigBtn').style.display=window.currentUser?.admin?'':'none';$('scoreBtn').onclick=()=>loadPjList('scores');$('selectPjBtn').onclick=()=>loadPjList('alive');$('newPjBtn').onclick=newCharacterFlow;$('continuePjBtn').onclick=()=>loadPjList('continue');$('multiBtn').onclick=showMultiplayerLobby;$('createWorldHubBtn').onclick=openWorldCreator;$('adminConfigBtn').onclick=()=>enterConfig()}
async function loadPjList(mode,targetId='characterHubList'){const box=document.getElementById(targetId);box.innerHTML='Cargando...';const q=mode==='scores'?'/api/user-pj':('/api/user-pj?nombre='+encodeURIComponent(window.currentUser.nombre)+(mode==='alive'||mode==='multi'?'&status=alive':''));const data=await fetch(q).then(r=>r.json()).catch(e=>({error:e.message}));if(data.error){box.textContent=data.error;return}let rows=Array.isArray(data)?data:[];if(mode==='continue')rows=rows.filter(r=>r.pj_status==='alive').sort((a,b)=>new Date(b.last_use||0)-new Date(a.last_use||0)).slice(0,1);box.innerHTML=rows.map(characterCard).join('')||'Sin personajes.';box.querySelectorAll('[data-pj]').forEach(b=>b.onclick=()=>{selectedCharacter=rows.find(r=>String(r.id)===b.dataset.pj);document.getElementById('characterHubOverlay')?.classList.add('hidden');document.getElementById('multiplayerOverlay')?.classList.add('hidden');openWorldCreator()})}
async function enterMultiplayerSession(){if(currentMultiSession)return;currentMultiSession=await fetch('/api/multi-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:window.currentUser?.id||window.currentUser?.nombre})}).then(r=>r.json()).catch(()=>null)}
async function logoutMultiplayer(){if(currentMultiSession?.id)await fetch('/api/multi-session?id='+currentMultiSession.id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:currentMultiSession.id})}).catch(()=>{});currentMultiSession=null;clearInterval(multiRefreshTimer);multiRefreshTimer=null;document.getElementById('multiplayerOverlay')?.classList.add('hidden');showCharacterHub()}
async function refreshMultiUsers(){const box=document.getElementById('multiUsersList');if(!box)return;const rows=await fetch('/api/multi-session').then(r=>r.json()).catch(()=>[]);box.innerHTML=(Array.isArray(rows)?rows:[]).map(r=>'<div class="multiUser"><b>'+r.user_id+'</b><span>'+new Date(r.login_time).toLocaleTimeString()+'</span></div>').join('')||'<p class="small">No hay otros usuarios en sala.</p>'}
async function showMultiplayerLobby(){multiplayerMode=true;await enterMultiplayerSession();let ov=document.getElementById('multiplayerOverlay');if(!ov){ov=document.createElement('div');ov.id='multiplayerOverlay';ov.className='overlay';ov.innerHTML='<div class="modal multiplayerModal"><div class="multiMain"><section><h2>MULTIPLAYER</h2><p class="small">Selecciona o crea un personaje vivo para unirte a una sesión compartida.</p><div class="hubActions compact"><button class="hubAction primary" id="multiSelectPjBtn"><b>Seleccionar personaje</b><span>Personajes vivos</span></button><button class="hubAction" id="multiNewPjBtn"><b>Nuevo personaje</b><span>Crear y volver aquí</span></button><button class="hubAction" id="multiContinueBtn"><b>Continuar sesión</b><span>Requiere jugadores conectados</span></button><button class="hubAction" id="multiCreateWorldBtn"><b>Crear mundo</b><span>Nueva sesión multiplayer</span></button><button class="hubAction danger" id="multiLogoutBtn"><b>Logout</b><span>Salir de la sala</span></button></div><div id="multiCharacterList" class="worldList hubList"></div></section><aside class="multiSidebar"><h3>Usuarios en sala</h3><div id="multiUsersList"></div></aside></div></div>';document.body.appendChild(ov)}document.getElementById('characterHubOverlay')?.classList.add('hidden');ov.classList.remove('hidden');multiSelectPjBtn.onclick=()=>loadPjList('multi','multiCharacterList');multiNewPjBtn.onclick=newCharacterFlow;multiContinueBtn.onclick=()=>loadPjList('multi','multiCharacterList');multiCreateWorldBtn.onclick=openWorldCreator;multiLogoutBtn.onclick=logoutMultiplayer;await refreshMultiUsers();clearInterval(multiRefreshTimer);multiRefreshTimer=setInterval(refreshMultiUsers,4000);loadPjList('multi','multiCharacterList')} 
...
const enterPlay=()=>{landingOverlay.classList.add('hidden');app.classList.remove('hidden');showCharacterHub();fetchConfigItems();fetchConfigClasses();fetchConfigFloors();fetchEnemyConfig();setupWorldSettings()};
...
loginForm.onsubmit=async e=>{e.preventDefault();loginBtn.disabled=true;loginStatus.textContent='Entrando...';try{const r=await fetch('/api/user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre:loginName.value,pass:loginPass.value})}),data=await r.json();if(!r.ok)throw new Error(data.error||'No se pudo iniciar sesión');window.currentUser=data;try{localStorage.setItem('mazmorraUser',JSON.stringify(data))}catch(err){}loginStatus.textContent=`Sesión iniciada: ${data.nombre}${data.admin?' · admin':''}`;enterPlay()}catch(err){loginStatus.textContent=err.message}finally{loginBtn.disabled=false}};
