const SUPABASE_TABLE='enemy_family';
...
function cleanFamily(body){
 const family=body.family_json||body;
 return {family_name:body.family_name??family.name??family.family_name??'Familia sin nombre',family_json:family};
}
function requestId(req){return req.query?.id||req.body?.id||req.body?.family_id||null}
module.exports=async(req,res)=>{
 try{
  ...
  if(req.method==='POST'){
   const row=cleanFamily(req.body||{});
  ...
  if(req.method==='PUT'){
   ...
   if(!id)return res.status(400).json({error:'Falta id para actualizar la familia'});
   const row=cleanFamily(req.body||{});
  ...
  if(req.method==='DELETE'){
   const id=requestId(req);
   if(!id)return res.status(400).json({error:'Falta id para borrar la familia'});
