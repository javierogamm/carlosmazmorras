...
function setupConfigMode(){renderConfigStatsHelp();const slotSel=document.getElementById('configSlot');if(slotSel&&!slotSel.options.length)slotSel.innerHTML=slots.map(s=>`<option value="${s}">${slotNames[s]}</option>`).join('');const imgInput=document.getElementById('configImageInput'),crop=document.getElementById('configCropCanvas'),preview=document.getElementById('configIconPreview');let img=null,rect=null;function clampRect(x,y){return{x:Math.max(0,Math.min(Math.round(x),Math.max(0,crop.width-50))),y:Math.max(0,Math.min(Math.round(y),Math.max(0,crop.height-50))),w:50,h:50}}function pointerPos(e){const r=crop.getBoundingClientRect();return{x:(e.clientX-r.left)*crop.width/r.width,y:(e.clientY-r.top)*crop.height/r.height}}function drawCrop(){const c=crop.getContext('2d');c.imageSmoothingEnabled=false;c.clearRect(0,0,crop.width,crop.height);c.fillStyle='#100b16';c.fillRect(0,0,crop.width,crop.height);if(img)c.drawImage(img,0,0);if(rect){c.save();c.strokeStyle='#ffd68b';c.lineWidth=2;c.strokeRect(rect.x+.5,rect.y+.5,50,50);c.fillStyle='#0008';c.fillRect(0,0,crop.width,rect.y);c.fillRect(0,rect.y+50,crop.width,crop.height-rect.y-50);c.fillRect(0,rect.y,rect.x,50);c.fillRect(rect.x+50,rect.y,crop.width-rect.x-50,50);c.restore()}}function saveIcon(){if(!img||!rect)return;const out=document.createElement('canvas');out.width=out.height=50;const o=out.getContext('2d');o.imageSmoothingEnabled=false;o.drawImage(img,rect.x,rect.y,50,50,0,0,50,50);preview.getContext('2d').drawImage(out,0,0);fetch(out.toDataURL('image/png')).then(r=>r.arrayBuffer()).then(buf=>{window.currentConfigIconHex=[...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join('');configIconStatus.textContent=`Icono 50x50 listo desde x:${rect.x}, y:${rect.y}`})}function moveBox(e){if(!img)return;const pos=pointerPos(e);rect=clampRect(pos.x-25,pos.y-25);drawCrop();saveIcon()}imgInput.onchange=()=>{const f=imgInput.files?.[0];if(!f)return;img=new Image();img.onload=()=>{crop.width=img.naturalWidth;crop.height=img.naturalHeight;rect=clampRect(0,0);drawCrop();saveIcon();configIconStatus.textContent=`Imagen original ${crop.width}x${crop.height}. Cuadro fijo 50x50.`};img.src=URL.createObjectURL(f)};crop.onpointerdown=e=>{crop.setPointerCapture?.(e.pointerId);moveBox(e)};crop.onpointermove=e=>{if(e.buttons)moveBox(e)};crop.onpointerup=e=>{crop.releasePointerCapture?.(e.pointerId);saveIcon()};saveConfigItemBtn.onclick=async()=>{configStatus.textContent='Guardando...';try{await saveConfigItems([{...currentConfigItemJson(),nombre:configNombre.value,slot:configSlot.value,tier:configTier.value,ilvl:configIlvl.value,item_json:currentConfigItemJson()}]);configStatus.textContent='Objeto guardado.'}catch(e){configStatus.textContent=e.message}};exportConfigJsonBtn.onclick=()=>{const blob=new Blob([JSON.stringify(currentConfigItemJson(),null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='config-item.json';a.click();URL.revokeObjectURL(a.href)};importConfigJsonInput.onchange=async()=>{const files=[...importConfigJsonInput.files];configStatus.textContent='Importando...';try{const items=[];for(const f of files){const parsed=JSON.parse(await f.text());items.push(...(Array.isArray(parsed)?parsed:[parsed]))}await saveConfigItems(items.map(x=>({...x,item_json:x})));configStatus.textContent=`Importados ${items.length} objeto(s).`}catch(e){configStatus.textContent=e.message}}}
