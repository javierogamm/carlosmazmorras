...
function setupClassConfigMode(){const imgInput=document.getElementById('configClassImageInput'),preview=document.getElementById('configClassIconPreview');if(!imgInput)return;configClassSelect.onchange=loadSelectedConfigClass;imgInput.onchange=()=>{const f=imgInput.files?.[0];if(!f)return;const img=new Image();img.onload=()=>{const out=document.createElement('canvas');out.width=out.height=50;const o=out.getContext('2d');o.imageSmoothingEnabled=false;const side=Math.min(img.naturalWidth,img.naturalHeight),sx=(img.naturalWidth-side)/2,sy=(img.naturalHeight-side)/2;o.drawImage(img,sx,sy,side,side,0,0,50,50);preview.getContext('2d').clearRect(0,0,50,50);preview.getContext('2d').drawImage(out,0,0);fetch(out.toDataURL('image/png')).then(r=>r.arrayBuffer()).then(buf=>{window.currentConfigClassIconHex=[...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join('');configClassIconStatus.textContent='Icono de clase preparado (50x50).'});};img.src=URL.createObjectURL(f)};saveConfigClassBtn.onclick=async()=>{configClassStatus.textContent='Guardando clase...';try{await saveConfigClass(currentConfigClassJson());configClassStatus.textContent='Clase guardada en BDD.'}catch(e){configClassStatus.textContent=e.message}};rollbackConfigClassBtn.onclick=async()=>{configClassStatus.textContent='Restaurando pixels por defecto...';try{window.currentConfigClassIconHex='';renderConfigIconPreview('','configClassIconPreview','configClassIconStatus');await saveConfigClass(currentConfigClassJson());configClassStatus.textContent='Rollback aplicado: la clase vuelve al sprite original.'}catch(e){configClassStatus.textContent=e.message}}}
