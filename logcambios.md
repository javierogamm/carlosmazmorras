...
- Unificada la fuente de verdad de pociones mediante el mismo filtro estricto `item_json.type === "potion"` para el loot, la lista de Configuración → Pociones, el selector de pociones iniciales de las clases y los cofres de poción.
- Los drops conservan una copia del objeto real almacenado en `config_items`, sin reescribir su tipo ni construir una definición de poción nueva, y escogen aleatoriamente entre las pociones del tier más alto permitido disponibles en ese catálogo.
- Sin ejecución de tests, conforme a la instrucción del usuario. Sincronizadas las versiones de runtime, paquete y cache-busting en `0.58.2`.
