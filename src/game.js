...
let selectedCharacter=null,selectedDungeonSession=null,multiplayerMode=false;
let configItems=[];
let configClasses=[];
let configFloors=[];
...
function showCharacterHub(){let ov=document.getElementById('characterHubOverlay');if(!ov){ov=document.createElement('div');ov.id='characterHubOverlay';ov.className='overlay';ov.innerHTML='<div class="modal"><h2>PERSONAJES Y SESIONES</h2><div class="startActions"><button id="scoreBtn">PUNTUACIONES</button><button id="selectPjBtn">SELECCIONAR PERSONAJE</button><button id="newPjBtn">NUEVO PERSONAJE</button><button id="continuePjBtn">CONTINUAR</button><button id="multiBtn">MULTIPLAYER</button><button id="adminConfigBtn">CONFIGURAR</button></div><div id="characterHubList" class="worldList"></div></div>';document.body.appendChild(ov)}ov.classList.remove('hidden');adminConfigBtn.style.display=window.currentUser?.admin?'':'none';scoreBtn.onclick=()=>loadPjList('scores');selectPjBtn.onclick=()=>loadPjList('alive');newPjBtn.onclick=()=>{selectedCharacter=null;ov.classList.add('hidden');dungeonOverlay.classList.remove('hidden');fetchDungeonWorlds()};continuePjBtn.onclick=()=>loadPjList('continue');multiBtn.onclick=()=>{multiplayerMode=true;loadPjList('multi')};adminConfigBtn.onclick=enterConfig}
async function loadPjList(mode){const box=document.getElementById('characterHubList');box.innerHTML='Cargando...';const q=mode==='scores'?'/api/user-pj':('/api/user-pj?nombre='+encodeURIComponent(window.currentUser.nombre)+(mode==='alive'||mode==='multi'?'&status=alive':''));const data=await fetch(q).then(r=>r.json()).catch(e=>({error:e.message}));if(data.error){box.textContent=data.error;return}let rows=Array.isArray(data)?data:[];if(mode==='continue')rows=rows.filter(r=>r.pj_status==='alive').sort((a,b)=>new Date(b.last_use||0)-new Date(a.last_use||0)).slice(0,1);box.innerHTML=rows.map(r=>'<button class="worldCard" data-pj="'+r.id+'"><b>'+ (r.pj_name||'Sin nombre')+'</b><span>'+r.nombre+' · '+(r.pj_status||'?')+' · score '+(r.pj_score||0)+'</span><small>'+(r.pj_json?.className||r.pj_json?.cls||'Clase ?')+' · '+(r.pj_json?.raceName||r.pj_json?.race||'Raza ?')+' · nivel '+(r.pj_json?.level||1)+'</small></button>').join('')||'Sin personajes.';box.querySelectorAll('[data-pj]').forEach(b=>b.onclick=()=>{selectedCharacter=rows.find(r=>String(r.id)===b.dataset.pj);document.getElementById('characterHubOverlay').classList.add('hidden');dungeonOverlay.classList.remove('hidden');fetchDungeonWorlds()})}
