...
- Reorganizados los modificadores de raza: en vez de bonus planos de vida/stamina/maná máximos, la raza aplica un coeficiente porcentual a la ganancia de esas tres stats al subir de nivel (p. ej. +20% de ganancia de vida por nivel). Eliminado el bonus "Curación por piso"; añadida en su lugar una regeneración de vida por turno, igual que las ya existentes de stamina y maná.
- Todos los modificadores de raza en % (efecto de habilidades, crítico, daño crítico, evasión, hallazgo de rareza, multiplicador de XP, gasto de recursos y las nuevas ganancias por nivel) se configuran ahora con un selector fijo de -100% a +200% en pasos de 10%, admitiendo valores negativos.
...
- **Calavera al morir.** Antes de cualquier pantalla de muerte se reproduce durante 2 s una calavera que sube girando sobre el cadáver. Se lanza desde los dos manejadores de muerte (normal y Soulseek), no desde `permanentDeath()`, porque a esa función también se llega al volver al menú o al guardar Soul Spikes y ahí una calavera no tendría sentido. Cuelga de `#gameStage` y no de `#combatFxLayer`, que es `overflow:hidden` y la recortaría al subir.
- **Ficha de raza en la creación.** Pulsar una tarjeta de raza ya no la selecciona en silencio: abre una ficha con el icono a 84 px, todos sus bonificadores de atributo y su habilidad racial (coste, enfriamiento y alcance); la raza se elige desde el botón de la propia ficha. `soulseek_shop.js` **reemplaza por completo** `renderSoulseekRaceChoices()` para filtrar por razas desbloqueadas, así que el cableado de las tarjetas se ha replicado también allí — sin eso el cambio no llegaba a ejecutarse nunca.
- **Icono del desbloqueo de raza más pequeño.** `styles.css` tiene un `canvas{width:100%}` global para el tablero que estiraba el icono de la ficha de la tienda a todo el ancho del modal: ese reescalado, y no el arte original, era lo que se veía pixelado. Ahora se fija a su tamaño natural. Solo la ficha de raza usa ese contenedor, de modo que las de clase y skill no cambian.
- **Vuelve la animación de «Generando mazmorra…».** El reloj de arena estaba atado a `soulseekStartCharacter()`, que desde el asistente de equipo inicial vuelve en cuanto se abre el modal, así que aparecía y desaparecía mucho antes de que se generase nada y el piso 1 se construía sin ningún aviso. Ahora envuelve `soulseekFinalizeLoadout()`, que es donde ocurre el trabajo real: hidratar los objetos elegidos, guardar el personaje y generar el piso.
- Añadido `[data-gamepad-cancel]`: una pantalla puede declarar su propio botón de «atrás» para que el botón B del mando la cierre, en lugar de tener que navegar hasta él. Usado por la ficha de raza.
- Verificación en navegador: poción usada sin abrir ninguna pantalla (efectos en el tablero, etiqueta flotante, consumida y curando); muerte con la calavera presente a 150 ms y el modal de resurrección todavía cerrado, y a los ~2,4 s la calavera retirada y el modal abierto; ficha de raza abriéndose desde la tarjeta con sus atributos y su habilidad, icono a 84×84 y selección aplicada al resumen. Recorrido completo del asistente de creación solo con mando pasando ahora por la ficha de raza. Repetidas las comprobaciones de equipo inicial (cinco catálogos), partida real y `test/interiors.test.js`.
- Actualizada la versión de runtime, paquete y cache-busting a `0.95.1`.

## v0.95.0 - 2026-08-20

- La creación de personaje de Soulseek pasa a ser un asistente de **tres pasos** (1 Raza · 2 Sexo · 3 Nombre) en vez de una única columna con la rejilla de razas, el sexo, la dificultad y el nombre apilados, que no cabía en pantalla. Reutiliza exactamente el mismo marcado y CSS que el asistente de creación normal (`.characterWizard` / `.wizardProgress` / `.wizardTrack` / `.wizardNav`): altura fija del modal y la rejilla de razas con su propio scroll dentro del paso, así que ya nada desborda.
- El paso 3 muestra un resumen de identidad (icono de la raza, raza, sexo y modo) antes del nombre, igual que hace la creación normal.
- Los ids de los elementos no han cambiado (`soulseekRaceChoices`, `soulseekGenderChoices`, `soulseekNameInput`, `soulseekCreateBtn`, `soulseekCreateBackBtn`), de modo que `soulseeker-dungeons.js` y `soulseek_shop.js` siguen funcionando sin tocarlos: el selector de **Dificultad** que inyectan junto a las opciones de sexo cae ahora dentro del paso 2.
- Soporte de mando del asistente:
  - Las tarjetas de raza (`[data-soulseek-race]`) ya son alcanzables: son `<div>`, así que había que declararlas tanto en el selector de navegación como en el que decide si un bloque cuenta como zona.
  - El mando ya no se mete en los pasos que están fuera de pantalla: el carrusel mantiene los tres pasos en el DOM y solo los desplaza, así que los inactivos se marcan `inert` y la navegación por foco los ignora. Esto arregla también el asistente de creación normal, que tenía el mismo problema.
  - Todo el asistente es una única zona de mando (`data-gamepad-zone`), de forma que la cruceta llega a SIGUIENTE y a CREAR PERSONAJE sin necesidad de cambiar de zona con los gatillos.
  - Nuevo punto de entrada declarativo `[data-gamepad-autofocus]`: al cambiar de paso el foco cae en la primera opción de ese paso (una raza, el sexo, el campo de nombre) en lugar de en las pestañas de progreso.
  - `markGamepadFocus()` hace focusable el elemento antes de enfocarlo. Las tarjetas de raza y de clase son `<div>` sin `tabindex`, y `.focus()` sobre ellas no hacía absolutamente nada; hasta ahora ese ajuste solo ocurría dentro del recorrido de la cruceta, no en el resto de caminos que mueven el foco.
  - El botón «ATRÁS» del paso 1 sale al menú de Soulseek: `.backToLanding` es `position:fixed` y el recorrido del mando se salta los elementos fijos (su `offsetParent` es `null`), así que era la única salida no alcanzable con mando.
- Verificación con mando simulado: recorrido completo del asistente usando solo primitivas de mando (raza → sexo → nombre) llegando a SIGUIENTE en los dos primeros pasos y a CREAR PERSONAJE en el tercero, con el foco entrando en la primera opción de cada paso; los pasos ocultos no aparecen nunca en la navegación; el modal no desborda (706 px de contenido en 706 px de alto); la dificultad se inyecta en el paso 2. Repetidas también las comprobaciones del asistente de equipo inicial (cinco catálogos), la partida real y los diálogos.
- Actualizada la versión de runtime, paquete y cache-busting a `0.95.0`.
